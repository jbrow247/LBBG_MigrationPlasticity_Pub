...
gull <- gull %>% group_by(id) %>% mutate(mn.mig = mean(mig.dist), max.mig = max(mig.dist), n.yrs = length(mig.dist)) %>% arrange(max.mig) %>% ungroup()
...
no.overlap <- filter(gull, !is.na(p.no.overlap)) %>% select(id) %>% distinct()

###linear model of range overlap with migration distance
wa.ol<-gull %>% group_by(id) %>% mutate(max.n.wa = max(n.winter.p)) %>% select(id, max.mig, mn_overlap, max_overlap, max.n.wa) %>% distinct()
wa.ol.lm <- lm(mn_overlap ~ max.mig, data = wa.ol)
...
bp.ol <- ggplot(rand.ol, aes(pair, overlap)) + geom_boxplot() +ylab("Distribution Overlap") + ggtitle("a")+theme(axis.title.y = element_text(size = 9),  axis.text = element_text(size = 9), axis.title.x = element_blank(),plot.title=element_text(hjust=0))
...
wtraj <- gull %>% group_by(id) %>% summarise(max.mig = max(mig.dist, na.rm = T)) %>% right_join(wtraj)
wtraj <- wtraj %>% mutate(v.scale = wvar/max.mig)
##spring
wtraj.s <- filter(wtraj, direction == "Spring")
lm.w.s<- lm(wvar ~ max.mig, data = wtraj.s)
...
lm.w.f <- lm(wvar ~ max.mig, data = wtraj.f)
anova(lm.w.f)
...
bp.tf <- ggplot(rand.t.f, aes(pair, mn_var)) + geom_boxplot()+ylab("Mean autumn route variance (km)") + ggtitle("b")+
  theme(axis.title.y = element_text(size = 9),  axis.text = element_text(size = 9), axis.title.x = element_blank(),plot.title=element_text(hjust=0))
bp.ts <- ggplot(rand.t.s, aes(pair, mn_var)) + geom_boxplot() +ylab("Mean spring route variance (km)") + ggtitle("c")+ theme(axis.title.y = element_text(size = 9),  axis.text = element_text(size = 9), axis.title.x = element_blank(),plot.title=element_text(hjust=0))
...
rand_traj_sum$max.mig <- rowMeans(rand_traj_sum[,c("md1","md2")])
rand_traj_sum$pair <- "between"

###combine within and between data sets into a single df for plotting later on
traj.s <- rbind(wtraj.s %>% select( mn_var = wvar, max.mig) %>% mutate(pair = "within"), 
                filter(rand_traj_sum, direction == "Spring") %>% select(mn_var, max.mig, pair)) %>% arrange(pair)
traj.f <- rbind(wtraj.f %>% select( mn_var = wvar, max.mig) %>% mutate(pair = "within"), 
                filter(rand_traj_sum, direction == "Autumn") %>% select(mn_var, max.mig, pair)) %>% arrange(pair)

lm(mn_var ~ max.mig, data = filter(traj.s, pair == 'between'))
lm(mn_var ~ max.mig, data = filter(traj.s, pair == 'within')) 
lm(mn_var ~ max.mig, data = filter(traj.f, pair == 'between')) 
lm(mn_var ~ max.mig, data = filter(traj.f, pair == 'within')) #
...
rand.UD_overlap_range$max.mig <- rowMeans(rand.UD_overlap_range[,c("md1","md2")])
rand.UD_overlap_range$pair <- "between"

###combine within and between data sets into a single df for plotting later on
ol <- rbind(rand.UD_overlap_range %>% select(overlap, max.mig, pair),
            wa.ol %>% ungroup() %>% select(overlap = mn_overlap, max.mig) %>% mutate(pair = "within"))
...
r.cd <- rptR::rpt(col.depart ~ (1 | id), grname = c("id"), data = gull, 
            datatype = "Gaussian", nboot = 1000, npermut = 0)
r.cd.full <- rptR::rpt(col.depart ~ sex + scale(mig.dist) + (1|col) + (1 | id), grname = c("id"), data = gull, 
            datatype = "Gaussian", nboot = 1000, npermut = 0)

lmm.cd <- lmer(col.depart ~ (1 | id), data = gull)
plot(lmm.cd)
lmm.cd.full <- lmer(col.depart ~ sex + scale(mig.dist) + (1|col) + (1 | id), data = gull)

##Individual repeatability in colony departure
idres.cd <- data.frame(id = gull$id, resid = resid(lmm.cd))
idvar.cd <- idres.cd %>% group_by(id) %>% summarise(idvar = mean(resid^2)) ##calculate indivdiuals residual variance
idvar.cd <- gull %>% select(id, max.mig) %>% distinct() %>% full_join(idvar.cd)
btwnvar <- VarCorr(lmm.cd)[[1]][1]
idvar.cd$r <- btwnvar/(btwnvar+idvar.cd$idvar)

idvar.cd <- gull %>% group_by(id) %>% summarise(range = max(col.depart) - min(col.depart)) %>% right_join(idvar.cd)

##linear model of indivdiual repeatability against migration distance
lm.cd <- lm(r~max.mig, data = idvar.cd)
lm.cd.n <- lm(r~1, data = idvar.cd)
lrt.cd <- anova(lm.cd.n, lm.cd) ## null

range.cd <- gull %>% group_by(id) %>% summarise(range = max(col.depart) - min(col.depart)) ## range in individual departure dates
```

```{r Winter arrival lmm}
r.wa <- rptR::rpt(winter.arrive ~ (1 | id) , grname = c("id"), data = gull, 
            datatype = "Gaussian", nboot = 1000, npermut = 0) ##population repeatability
r.wa.full <- rptR::rpt(winter.arrive ~ sex + mig.dist + (1|col) + (1 | id), grname = c("id"), data = gull, 
            datatype = "Gaussian", nboot = 1000, npermut = 0)

lmm.wa <- lmer(winter.arrive ~ (1 | id), data = gull)
plot(lmm.wa)
lmm.wa.full <- lmer(winter.arrive ~ sex + scale(mig.dist) + (1|col) + (1 | id), data = gull)
##individual repeatability
idres.wa <- data.frame(id = gull$id, resid = resid(lmm.wa))
idvar.wa <- idres.wa %>% group_by(id) %>% summarise(idvar = mean(resid^2)) ##calculate indivdiuals residual variance
idvar.wa <- gull %>% select(id, max.mig) %>% distinct() %>% full_join(idvar.wa)
btwnvar <- VarCorr(lmm.wa)[[1]][1]
idvar.wa$r <- btwnvar/(btwnvar+idvar.wa$idvar)

##Does individual repeatability increase with mgiration distance?
lm.wa <- lm(r~max.mig, data = idvar.wa)
lm.wa.n <- lm(r~1, data = idvar.wa)
lrt.wa <- anova(lm.wa.n, lm.wa) ## null

fall.mig.time <- gull %>% mutate(mig.time.f = winter.arrive - col.depart) %>% summarise(mn = mean(mig.time.f), med = median(mig.time.f), max = max(mig.time.f), min = min(mig.time.f)) ## duration between col departure and winter arrive

range.wa <- gull %>% group_by(id) %>% summarise(range = max(winter.arrive) - min(winter.arrive)) ## range in indivdiual arrival dates
```

```{r Winter departure}
r.wd <- rptR::rpt(winter.depart ~ (1 | id), grname = c("id"), data = gull, 
            datatype = "Gaussian", nboot = 1000, npermut = 0)##population repeatability
r.wd.full <- rptR::rpt(winter.depart ~ sex + mig.dist + (1|col) + (1 | id), grname = c("id"), data = gull, 
            datatype = "Gaussian", nboot = 1000, npermut = 0)
##individual repeatability
lmm.wd <- lmer(winter.depart ~ (1 | id), data = gull)
plot(lmm.wd)
...
idvar.wd <- gull %>% select(id, max.mig) %>% distinct() %>% full_join(idvar.wd)
...
lm.wd <- lm(r~max.mig, data = idvar.wd)
...
##population repeatability
r.ca <- rptR::rpt(col.arrival ~ (1 | id), grname = c("id"), data = gull, 
            datatype = "Gaussian", nboot = 1000, npermut = 0)

r.ca.full <- rptR::rpt(col.arrival ~ sex + mig.dist + (1|col) + (1 | id), grname = c("id"), data = gull, 
            datatype = "Gaussian", nboot = 1000, npermut = 0)

lmm.ca <- lmer(col.arrival ~ (1 | id), data = gull)
plot(lmm.ca)
...
idvar.ca <- gull %>% select(id, max.mig) %>% distinct() %>% full_join(idvar.ca)
btwnvar <- VarCorr(lmm.ca)[[1]][1]
idvar.ca$r <- btwnvar/(btwnvar+idvar.ca$idvar)

##does individual repeatability change with migration distance?
lm.ca <- lm(r~max.mig, data = idvar.ca)
...
##Objective 1: How variable is individual behaviour in relation to the variation among individuals?

The 50% UDs of nonbreeding season tracks indicated that individuals used between `r min(gull$n.winter.p)` &ndash; `r max(gull$n.winter.p)` core areas during the nonbreeding season. Wintering area polygons from different nonbreeding seasons overlapped for all individuals (n = `r length(gull$id) - 5`) with the exception of five. Wintering areas for these individual moved from France to Western Sahara, Mauritania to Portugal, France to UK and 2 individuals moved from Morocco and UK. `r length(no.overlap) - 5` individuals used a stopover in one year that did not overlap with a polygon in another year.  Mean BA overlap in 95% UDs ranged between `r myround(min(wa.ol$mn_overlap),2)` &ndash; `r myround(max(wa.ol$mn_overlap),2)` (median = `r myround(median(wa.ol$mn_overlap),2)`). UDs were significantly more similar within individuals across years than between individuals using similar migration strategies (median between-individual BA = `r myround(mdb.ol, 2)`, Fig. 2a and S2 in supplementary material), with none of the randomized sets producing a difference in median more extreme than the actual data (p = `r myround(pExtreme.ol, 3)`). 

Variation in migration route was generally low in both autumn (range =  `r  round(min(wtraj.f$wvar),0)` &ndash; `r round(max(wtraj.f$wvar),0)` km, median = `r round(mdw.tf,0)` km) and spring (range =  `r round(min(wtraj.s$wvar),0)` &ndash; `r round(max(wtraj.s$wvar),0)` km, median = `r round(mdw.ts,0)` km). Most within-individual route variation occurred at the Bay of Biscay crossing (~ 500 km at the widest N-S crossing) and over the arid centre of Spain (also around 400 km directly N-S; Fig. 1). Within-individual variation was significantly lower than variation between individuals using similar start and end destinations (autumn: between individual median = `r round(mdb.tf,0)` km; spring: between individual median = `r round(mdb.ts,0)` km; Fig. 2b-c and S2 in supplementary material) with probabilities of less than `r round(pExtreme.tf,0)` and `r signif(pExtreme.ts,3)`, of obtaining a difference in medians as extreme from a randomized set.

Individuals departed their breeding colonies between `r jultodate(min(gull$col.depart))` &ndash; `r jultodate(max(gull$col.depart))` (median = `r jultodate(median(gull$col.depart))`; Fig. 3a). The median range of departure dates within an individual was `r median(range.cd$range)` days (range: `r min(range.cd$range)` &ndash;  `r max(range.cd$range)`) and was highly repeatable (`r myround(r.cd$R,2)` [`r myround(r.cd$CI_emp[1],2)` - `r myround(r.cd$CI_emp[2],2)`]).Arrival to wintering area occurred between `r jultodate(min(gull$winter.arrive))` &ndash;  `r jultodate(max(gull$winter.arrive))` (median = `r jultodate(median(gull$winter.arrive))`; Fig. sb). Within-individuals arrival dates to wintering area ranged across years by  `r min(range.wa$range)` &ndash;  `r max(range.wa$range)` days (median = `r median(range.wa$range)`) and repeatability was high (`r myround(r.wa$R,2)` [`r myround(r.wa$CI_emp[1],2)`  - `r myround(r.wa$CI_emp[2],2)`]). During spring, the range of arrival and departure dates within individuals was more narrow, and repeatability values were higher. Individuals departed their wintering areas between `r jultodate(min(gull$winter.depart))` - `r jultodate(max(gull$winter.depart))` (median = `r jultodate(median(gull$winter.depart))`; Fig. 1c). The median range of departure dates within an individual was `r median(range.wd$range)` days (range: `r min(range.wd$range)` &ndash;  `r max(range.wd$range)`). Repeatability of departure from wintering area was high (`r myround(r.wd$R,2)` [`r myround(r.wd$CI_emp[1],2)` - `r myround(r.wd$CI_emp[2],2)`], complete model = `r myround(r.wd.full$R,2)`). Arrival to breeding colonies occurred between `r jultodate(min(gull$col.arrival))` &ndash;  `r jultodate(max(gull$col.arrival))` (median = `r jultodate(median(gull$col.arrival))`; Fig. 1d). Median range in individual arrival dates to colony was `r min(range.ca$range)` &ndash;  `r max(range.ca$range)` days (median = `r median(range.ca$range)`). Repeatability was also high (`r myround(r.ca$R,2)` [`r myround(r.ca$CI_emp[1],2)` - `r myround(r.ca$CI_emp[2],2)`], complete model = `r myround(r.ca.full$R,2)`). Complete models likely differed from simple models during spring because migration distance explained a large amount of the between-individual variation in spring arrival and departure times. Linear mixed models and the variance partitioned toward the individual random effect and random error are reported in the supplementary material. 

While the majority of individuals tended to be highly consistent, for each behaviour a few individuals demonstrated high variability (e.g. Fig 1, Fig S3 in supplementary material).  The individuals demonstrating the most variability were not consistent across behaviours: `r n.variable` different individuals (`r myround(100*n.variable/length(unique(gull$id)),1)`%) were among the upper 5th percentile for variability in at least one of the 7 behaviours analysed (Table Sx in supplementary material). 

##Objective 2: Influence of migration distance on behavioural variation

Migration distances ranged from `r round(min(gull$mig.dist),0)` &ndash; `r round(max(gull$mig.dist),0)` km (median = `r round(median(gull$mig.dist),0)` km). Nonbreeding distribution overlap decreased with migration distance (overlap = `r myround(wa.ol.lm$coefficients[1],3)` + `r myround(wa.ol.lm$coefficients[2],6)`·migration distance ,LRT: F = `r myround(lrt.ol$"F"[2],3)`, p = `r myround(lrt.ol$"Pr(>F)"[2],3)`; Fig. 2a) and within-individual route variation increased significantly with migration distance in spring, (variation = `r myround(lm.w.s$coefficients[1],3)` + `rmyround(lm.w.s$coefficients[2],3)`·migration distance ) LRT: F = `r myround(lrt.w.s$"F"[2],3)`, p = `r myround(lrt.w.s$"Pr(>F)"[2],3)`; Fig. 2c), but not autumn autumn (LRT: F = `r myround(lrt.w.f$"F"[2],3)`, p = `r myround(lrt.w.f$"Pr(>F)"[2],3)`; Fig. 2b). However, between-individual overlap and route variation also increased (Fig. 2a-c), indicating within-individual variability in relation to between-individual variation likely only changes slightly, if at all, with migration distance. Migration distance did not correlate with departure date from colony (LRT: F = `r myround(lrt.cd$"F"[2],3)`, p = `r myround(lrt.cd$"Pr(>F)"[2],3)`; Fig. 2d), arrival to wintering area (LRT: F = `r myround(lrt.wa$"F"[2],3)`, p = `r myround(lrt.wa$"Pr(>F)"[2],3)`; Fig. 2e), departure from wintering area (LRT: F = `r myround(lrt.wd$"F"[2],3)`, p = `r myround(lrt.wd$"Pr(>F)"[2],3)`; Fig. 2f) or date of arrival to colony (LRT: F = `r myround(lrt.ca$"F"[2],3)`, p = `r myround(lrt.ca$"Pr(>F)"[2],3)`; Fig. 2g).
...
tiff("Fig1.tiff", height = 12.5, width = 12, units = "cm", res = 300)
p.mig.var
dev.off()
...
wa.ol.lm.b <- lm(overlap ~ max.mig, data = filter(ol, pair == 'between'))

p.ol <- ggplot(ol, aes(max.mig, overlap, col = pair)) + geom_point(size = 1) + geom_smooth(method = "lm",se=F, size = 1) + scale_color_manual(values = c("grey75", "black"))  + xlab("Individual maximum migration distance (km)")  + ylab("Distribution Overlap") + scale_x_continuous(expand = c(0, 0)) + ggtitle('a') +
  theme_bw() +  theme(axis.title.x = element_blank(), axis.text =element_text(size=9),axis.title.y=element_text(size=9), legend.position = "none", panel.grid=element_blank(), plot.title=element_text(hjust=0), plot.background = element_blank(), plot.margin = margin(-3, 3, -3, 3)) ##with 95% confidence interval

p.st <- ggplot(traj.s, aes(max.mig, mn_var, col = pair)) + geom_point(size = 1) + geom_smooth(method = "lm", se=F, size = 1) +
  scale_color_manual(values = c("grey75", "black"))+ ggtitle('c') +
...
p.ft<-ggplot(traj.f, aes(max.mig, mn_var, col = pair)) + geom_point(size = 1) + 
  scale_color_manual(values = c("grey75", "black")) + 
    xlab("Individual maximum migration distance (km)") + ggtitle('b') +
...
p.cd <- ggplot(idvar.cd, aes(max.mig, r)) + geom_point(size = 1) +  ylab(expression(paste(R[i], "  of colony departures"))) + ylim(0,1) + scale_x_continuous(expand = c(0, 0)) + ggtitle('d') +
  xlab("Individual maximum migration distance (km)") + theme_bw() +    theme(axis.title.x=element_blank(),axis.text =element_text(size=9),axis.title.y=element_text(size=9), panel.grid=element_blank(), plot.title=element_text(hjust=0), plot.background = element_blank(), plot.margin = margin(-3, 3, -3, 3)) 

p.wa <- ggplot(idvar.wa, aes(max.mig, r)) + geom_point(size = 1) +  ylab(expression(paste(R[i], "  of winter arrivals"))) + ylim(0,1) + scale_x_continuous(expand = c(0, 0)) + ggtitle('e') +
  xlab("Individual maximum migration distance (km)") + theme_bw() +    theme(axis.title.x = element_blank(),axis.text =element_text(size=9),axis.title.y=element_text(size=9),plot.background = element_blank(), panel.grid=element_blank(), plot.title=element_text(hjust=0), plot.margin = margin(-3, 3, -3, 3))

p.wd <- ggplot(idvar.wd, aes(max.mig, r)) + geom_point(size = 1) +  ylab(expression(paste(R[i], "  of winter departures"))) + ylim(0,1) + scale_x_continuous(expand = c(0, 0)) + ggtitle('f') +
  xlab("Maximum migration distance (km)") + theme_bw() +   theme(axis.text =element_text(size=9),axis.title=element_text(size=9), plot.background = element_blank(), panel.grid=element_blank(), plot.title=element_text(hjust=0), plot.margin = margin(-3, 3, 0, 3)) 

p.ca <- ggplot(idvar.ca, aes(max.mig, r)) + geom_point(size = 1) +  ylab(expression(paste(R[i], "  of colony arrivals"))) + ylim(0,1) + ggtitle('g') + scale_x_continuous(expand = c(0, 0)) +
  xlab("Maximum migration distance (km)") + theme_bw() +   theme(axis.text =element_text(size=9),axis.title=element_text(size=9), plot.background = element_blank(), panel.grid=element_blank(), plot.title=element_text(hjust=0), plot.margin = margin(-3, 3, 0, 3)) 

tiff("Fig2.tiff",  width = 16.25, height = 20, units = "cm", res = 300)
plot_grid(p.ol, NULL, p.ft, p.st, p.cd, p.wa, p.wd, p.ca, nrow = 4, rel_heights = c(1,1,1,1.15))
dev.off()
...
```{r Fig. 3, include=T}
p.r.cd <- ggplot(gull, aes(x = reorder(id, mig.dist, max), as.Date(col.depart-1, "1970-01-01"))) + 
  geom_boxplot() + annotate("text", x = 23, y = as.Date(min(gull$col.depart), "1970-01-01"), size = 3,label = paste0("R = ", myround(r.cd$R,2), "[", myround(r.cd$CI_emp[1],2), "\226", myround(r.cd$CI_emp[2],2), "]")) + 
  ...
  ylab("Day of departure from colony") + xlab(expression("Short migration"%<-% "Individual" %->% "Long migration")) + theme_bw() +  
   theme(axis.text.x=element_blank(), axis.title.x = element_blank(), axis.text.y =element_text(size=8),axis.title.y=element_text(size=9),panel.grid=element_blank(),legend.position = "none")  

p.r.wa <- ggplot(gull, aes(x = reorder(id, mig.dist, max), as.Date(winter.arrive-1, "1970-01-01"))) + 
  geom_boxplot() +  
  annotate("text", x = 65, y = as.Date(min(gull$winter.arrive), "1970-01-01"), size = 3,
           label =paste0("R = ", myround(r.wa$R,2), "[", myround(r.wa$CI_emp[1],2), "\226", myround(r.wa$CI_emp[2],2), "]")) +
  scale_y_date(breaks = as.Date(c(1,32,60,91,121,152,182,213,244,274,305,335,366, 397, 425, 456, 486, 517, 547)-1, "1970-01-01"), date_labels = "%b") +
  ylab("Day of arrival to winter area") + xlab(expression("Short migration"%<-% "Individual" %->% "Long migration")) + theme_bw() +  
    theme(axis.text.x=element_blank(), axis.title.x = element_blank(), axis.text.y =element_text(size=8),axis.title.y=element_text(size=9),panel.grid=element_blank(),legend.position = "none")  

p.r.wd <- ggplot(gull, aes(x = reorder(id, mig.dist, max), as.Date(winter.depart-1, "1970-01-01"))) + 
  geom_boxplot() +  annotate("text", x = 23, y = as.Date(min(gull$winter.depart), "1970-01-01"), size = 3, 
                             label = paste0("R = ", myround(r.wd$R,2), "[", myround(r.wd$CI_emp[1],2), "\226", myround(r.wd$CI_emp[2],2), "]")) + 
  scale_y_date(breaks = as.Date(c(1,32,60,91,121,152,182,213,244,274,305,335,366, 397, 425, 456, 486, 517, 547)-1, "1970-01-01"), date_labels = "%b") +
  ylab("Day of departure from winter area    ") + xlab(expression("Short migration"%<-% "Individual" %->% "Long migration")) + theme_bw() +  
  theme(axis.text.x=element_blank(), axis.title.x=element_text(size=9), axis.text.y =element_text(size=8),axis.title.y=element_text(size=9),panel.grid=element_blank(),legend.position = "none") 

p.r.ca <- ggplot(gull, aes(x = reorder(id, mig.dist, max), as.Date(col.arrival-1, "1970-01-01"))) + 
  geom_boxplot() +  annotate("text", x = 65, y = as.Date(min(gull$col.arrival), "1970-01-01"), size = 3, label = paste0("R = ", myround(r.ca$R,2), "[", myround(r.ca$CI_emp[1],2), "\226", myround(r.ca$CI_emp[2],2), "]")) + 
  scale_y_date(breaks = as.Date(c(1,32,60,91,121,152,182,213,244,274,305,335,366, 397, 425, 456, 486, 517, 547)-1, "1970-01-01"), date_labels = "%b") +
  ylab("Day of arrival to colony ") + xlab(expression("Short migration"%<-% "Individual" %->% "Long migration"))  + theme_bw() +  
  theme(axis.text.x=element_blank(), axis.text.y =element_text(size=8),axis.title.y=element_text(size=9), axis.title.x=element_text(size=9), panel.grid=element_blank(), legend.position = "none")

tiff("Fig3.tiff",  width = 16, height = 12, units = "cm", res = 300)
plot_grid(p.r.cd, p.r.wa, p.r.wd, p.r.ca, nrow = 2, rel_heights = c(1,1.1), labels = "auto")
dev.off()
...
```{r Fig. S2, include = T}
png("Figs2.png",  width = 16, height = 7.33, units = "cm", res = 300)
plot_grid(bp.ol, bp.tf, bp.ts, nrow = 1)
dev.off()
```
...
plot_grid(p.hol, NULL, p.htf, p.hts, p.hcd, p.hwa, p.hwd, p.hca, nrow = 4)
 dev.off()
