...
#setwd("C:/Users/jbrown1/surfdrive/PhD/Research/Plasticity of migratory strategies/R/Publication_version/Plasticity_pub")
library(lme4)
library(lubridate)
library(tidyverse)
library(rptR)
...
r.md <- rptR::rpt(mig.dist ~ (1 | id) + (1 | col), grname = c("id"), data = gull, 
...
```{r wa site fidelity}
sf.ol<-gull %>% filter(change.wa == F) %>% group_by(id) %>% select(id, med.mig, sf_mn_overlap) %>% distinct()
...
lm.data <-  gull %>% filter(change.wa == F) %>% group_by(id) %>% mutate(max.n.wa = max(n.winter.p)) %>% select(id, med.mig, mn_overlap, max_overlap, max.n.wa) %>% distinct()
...
ol.df <- gull %>% filter(change.wa == F) %>% select(id, mn_overlap, mig.dist) %>% group_by(id) %>% summarise(mn_overlap = unique(mn_overlap), med.mig = median(mig.dist))
...
lmm.cd.nowa <- lmer(col.depart ~ mig.dist + (1|col) + (1 | id), data = filter(gull, !is.na(col.depart) & change.wa == F) %>% mutate(mig.dist = mig.dist/1000)) ##remove wa changers
idres.cd <- data.frame(id = filter(gull, !is.na(col.depart)& change.wa == F)$id, resid = resid(lmm.cd.nowa))
idvar.cd <- idres.cd %>% group_by(id) %>% summarise(idvar = mean(resid^2)) ##calculate indivdiuals residual variance
idvar.cd <- filter(gull, !is.na(col.depart)& change.wa == F)%>% select(id, med.mig) %>% distinct() %>% full_join(idvar.cd)
btwnvar <- VarCorr(lmm.cd.nowa)[[1]][1]
idvar.cd$r <- btwnvar/(btwnvar+idvar.cd$idvar)
idvar.cd <- filter(gull, !is.na(col.depart)& change.wa == F) %>% group_by(id) %>% summarise(range = max(col.depart) - min(col.depart)) %>% right_join(idvar.cd)
...
lmm.wa.nowa <- lmer(winter.arrive ~ mig.dist + (1|col) +  (1 | id), data = filter(gull, !is.na(winter.arrive)& change.wa == F)%>% mutate(mig.dist = mig.dist/1000))
idres.wa <- data.frame(id = filter(gull, !is.na(winter.arrive) & change.wa == F)$id, resid = resid(lmm.wa.nowa ))
idvar.wa <- idres.wa %>% group_by(id) %>% summarise(idvar = mean(resid^2)) ##calculate indivdiuals residual variance
idvar.wa <- filter(gull, !is.na(winter.arrive)& change.wa == F) %>% select(id, med.mig) %>% distinct() %>% full_join(idvar.wa)
btwnvar <- VarCorr(lmm.wa.nowa )[[1]][1]
...
lmm.wd.nowa <- lmer(winter.depart ~ mig.dist + (1|col) + (1 | id), data = gull %>% filter(change.wa == F) %>% mutate(mig.dist = mig.dist/1000))
idres.wd <- data.frame(id = (gull %>% filter(change.wa == F))$id, resid = resid(lmm.wd.nowa))
idvar.wd <- idres.wd %>% group_by(id) %>% summarise(idvar = mean(resid^2)) ##calculate indivdiuals residual variance
idvar.wd <- gull %>% filter(change.wa == F) %>% select(id, med.mig) %>% distinct() %>% full_join(idvar.wd)
btwnvar <- VarCorr(lmm.wd.nowa)[[1]][1]
...
lmm.ca.nowa <- lmer(col.arrival ~ mig.dist + (1|col) + (1 | id), data = gull %>% filter(change.wa == F) %>% mutate(mig.dist = mig.dist/1000))
idres.ca <- data.frame(id = (gull %>% filter(change.wa == F))$id, resid = resid(lmm.ca.nowa))
idvar.ca <- idres.ca %>% group_by(id) %>% summarise(idvar = mean(resid^2)) ##calculate indivdiuals residual variance
idvar.ca <- gull %>% filter(change.wa == F) %>% select(id, med.mig) %>% distinct() %>% full_join(idvar.ca)
btwnvar <- VarCorr(lmm.ca.nowa)[[1]][1]
...
###id w changing wa areas removed
sf.df <- gull %>% filter(change.wa == F) %>% select(id, sf_mn_overlap, mig.dist) %>% group_by(id) %>% summarise(sf_mn_overlap = unique(sf_mn_overlap), med.mig = median(mig.dist))
...
p.sf <- ggplot(sf.df, aes(med.mig, sf_mn_overlap)) + geom_point(size = 1)   +  xlab("Individual median migration distance (km)")  + ylab("Winter area overlap") + scale_x_continuous(expand = c(0, 0)) + ggtitle('b') +
  theme_bw() +  theme(axis.title.x = element_blank(), axis.text =element_text(size=9),axis.title.y=element_text(size=9), legend.position = "none", panel.grid=element_blank(), plot.title=element_text(hjust=0), plot.background = element_blank(), plot.margin = margin(0, 3, -3, 3)) ##with 95% confidence interval
...
as.character((filter(sf.df, sf_mn_overlap <= quantile(sf.df$sf_mn_overlap, .05)) %>% select(id,sf_mn_overlap) %>% arrange(sf_mn_overlap))$id),
