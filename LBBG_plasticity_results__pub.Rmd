...
gull <- gull %>% group_by(id) %>% mutate(med.mig = mean(mig.dist), med.mig = median(mig.dist), n.yrs = length(mig.dist)) %>% arrange(med.mig) %>% ungroup()
...
lrt.sf <- lmtest::lrtest(sf.lm.0, sf.lm)
...
lrt.ol <- lmtest::lrtest(wa.ol.lm.0, wa.ol.lm)
...
anova(lm.w.s)
lrt.w.s <- lmtest::lrtest(null,lm.w.s)

##Autumn
wtraj.f <- filter(wtraj, direction != "Spring")
lm.w.f <- lm(wvar ~ med.mig, data = wtraj.f)
anova(lm.w.f)
lrt.w.f <- lmtest::lrtest(lm(wvar ~ 1, data = wtraj.f), lm.w.f)
...
lrt.dtraj.s <- lmtest::lrtest(dtraj.s.lm.0, dtraj.s.lm) ## residual overlap does not significantly increase with distance
...
lrt.dtraj.f <- lmtest::lrtest(dtraj.f.lm.0, dtraj.f.lm) ## residual overlap does not significantly increase with distance
...
lrt.dol <- lmtest::lrtest(wa.dol.lm.0, wa.dol.lm) ## residual overlap does significantly increase with distance
...
lrt.cd <- lmtest::lrtest(lm.cd.n, lm.cd) ## null
...
lrt.wa <- lmtest::lrtest(lm.wa.n, lm.wa) ## null
...
lrt.wd <- lmtest::lrtest(lm.wd.n, lm.wd) 
...
lrt.ca <- lmtest::lrtest(lm.ca.n, lm.ca)
...
Migration distances ranged from `r round(min(gull$mig.dist),0)` &ndash; `r round(max(gull$mig.dist),0)` km (median = `r round(median(gull$mig.dist),0)` km). Intra-individual non-breeding distribution overlap decreased with migration distance (overlap = `r myround(wa.ol.lm$coefficients[1],3)` + `r myround(wa.ol.lm$coefficients[2],6)`·migration distance, LRT: X2(`r lrt.ol$Df[2]`) = `r myround(lrt.ol$Chisq [2],3)`, p = `r myround(lrt.ol$"Pr(>Chisq)"[2],3)`; Fig. 2a). However, intra-individual variation increased at a significantly lower rate than the increase between individuals (residual overlap = `r myround(wa.dol.lm$coefficients[1],3)` + `r myround(wa.dol.lm$coefficients[2],3)`·migration distance, LRT: X2(`r lrt.dol$Df[2]`) = `r myround(lrt.dol$Chisq[2],3)`, p = `r myround(lrt.dol$"Pr(>Chisq)"[2],3)`; Fig s6a in supplemental material), suggesting that longer distance migrants were less variable in their behaviour than expected when considering the total space traversed during their movements (and vice-versa for shorter distance migrants). Intra-individual route variation increased significantly with migration distance in spring (variation = `r myround(lm.w.s$coefficients[1],3)` + `r myround(lm.w.s$coefficients[2],3)`·migration distance, LRT: X2(`r lrt.w.s$Df[2]`) = `r myround(lrt.w.s$Chisq[2],3)`, p = `r myround(lrt.w.s$"Pr(>Chisq)"[2],3)`; Fig. 2d), but not in autumn but not autumn (LRT: X2(`r lrt.w.s$Df[2]`) = `r myround(lrt.w.f$Chisq[2],3)`, p = `r myround(lrt.w.f$"Pr(>Chisq)"[2],3)`; Fig. 2c). Changes in route variation with migration distance did not, however, significantly differ from those observed between individuals (autumn LRT: X2(`r lrt.dtraj.f$Df[2]`) = `r myround(lrt.dtraj.f$Chisq[2],3)`, p = `r myround(lrt.dtraj.f$"Pr(>Chisq)"[2],3)`, spring LRT: X2(`r lrt.dtraj.s$Df[2]`) = `r myround(lrt.dtraj.s$Chisq[2],3)`, p = `r myround(lrt.dtraj.s$"Pr(>Chisq)"[2],3)`, Fig S6b-c in supplemental material).


Winter site fidelity did not change significantly with migration distance (LRT: X2(`r lrt.sf$Df[2]`) = `r myround(lrt.sf$Chisq[2],3)`, p = `r myround(lrt.sf$"Pr(>Chisq)"[2],3)`, Fig2b). Migration distance also did not correlate with individual repeatability in departure date from colony (LRT: X2(`r lrt.cd$Df[2]`) = `r myround(lrt.cd$Chisq[2],3)`, p = `r myround(lrt.cd$"Pr(>Chisq)"[2],3)`; Fig. 2e), arrival to wintering area (LRT: X2(`r lrt.wa$Df[2]`) = `r myround(lrt.wa$Chisq[2],3)`, p = `r myround(lrt.wa$"Pr(>Chisq)"[2],3)`; Fig. 2f), departure from wintering area (LRT: X2(`r lrt.wd$Df[2]`) = `r myround(lrt.wd$Chisq[2],3)`, p = `r myround(lrt.wd$"Pr(>Chisq)"[2],3)`; Fig. 2g) or arrival to colony (LRT: X2(`r lrt.ca$Df[2]`) = `r myround(lrt.ca$Chisq[2],3)`, p = `r myround(lrt.ca$"Pr(>Chisq)"[2],3)`; Fig. 2h), suggesting individual consistency of migration timing did not increase with migration distance. 
...
p.olr <- ggplot(wa.ol, aes(med.mig, resid.var)) + geom_point(size = 1) + geom_smooth(method = "lm",se=F, size = 1, col = "black") + xlab("Median migration distance (km)")  + ylab("Residual Distribution Overlap") + scale_x_continuous(expand = c(0, 0)) + ggtitle('a') +
