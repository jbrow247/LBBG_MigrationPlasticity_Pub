...
###stopover times
en.ex.noagg <- readRDS("en.ex.RDS")
en.ex.noagg <- en.ex.noagg %>% ungroup() 
stopover.time.f <- en.ex.noagg %>% left_join(gull %>% select(id, birdyear, col.depart, winter.arrive)) %>%
  filter(enter >= col.depart & exit <= winter.arrive) %>% ungroup() %>% summarise(so.time.f = sum(time.in.poly))
  
stopover.time.s <- en.ex.noagg %>% left_join(gull %>% select(id, birdyear, col.arrival, winter.depart)) %>%
...
gull$med.mig <- ifelse(gull$change.wa == T, NA, gull$med.mig)
gull$mn.mig <- ifelse(gull$change.wa == T, NA, gull$mn.mig)
...
r.md <- rpt(mig.dist ~ (1 | id)  + (1 | col), grname = c("id"), data = gull, 
...
```{r wa site fidelity}
sf.ol<-gull %>%
  group_by(id) %>% select(id, med.mig, sf_mn_overlap) %>% distinct()
data <- filter(sf.ol, !is.na(med.mig))
sf.lm <- lm(sf_mn_overlap ~ med.mig, data = data)
sf.lm.0 <- lm(sf_mn_overlap ~ 1, data = data)
...
##remove wa change for lm with mig dist
lm.data <-  filter(wa.ol, !is.na(med.mig))
wa.ol.lm <- lm(mn_overlap ~ med.mig, data = lm.data)
wa.ol.lm.0 <- lm(mn_overlap ~ 1, data = lm.data)
...
##no traj for birds who changed wa
...
rand_traj_sum <-  gull %>% select(p.id_birdyear = id_birdyear, md2 = mig.dist) %>% right_join(rand_traj_sum)
...
###id w changing wa areas removed
ol.df <- gull %>% mutate(mig.dist = ifelse(change.wa == T, NA, mig.dist)) %>% select(id, mn_overlap, mig.dist) %>% group_by(id) %>% summarise(mn_overlap = unique(mn_overlap), med.mig = median(mig.dist)) %>% ungroup() 
ol.df$null <- ol.btwn.m*ol.df$med.mig + ol.btwn.b  
ol.df$resid.var <- ol.df$mn_overlap - ol.df$null

##check for significance
wa.dol.lm <- lm(resid.var ~ med.mig, data = ol.df)
wa.dol.lm.0 <- lm(resid.var ~ 1, data = ol.df)
lrt.dol <- anova(wa.dol.lm.0, wa.dol.lm) ## residual overlap does significantly increase with distance

##plot
ggplot(ol.df, aes(med.mig, resid.var)) + geom_point()
...
data <- filter(idvar.cd, !is.na(med.mig))
lm.cd <- lm(r~med.mig, data = data)
lm.cd.n <- lm(r~1, data = data)
lrt.cd <- anova(lm.cd.n, lm.cd) ## null
```

```{r Winter arrival lmm}
r.wa <- rptR::rpt(winter.arrive ~ (1|col) + (1 | id) , grname = c("id"), data = filter(gull, !is.na(winter.arrive)) %>% mutate(mig.dist = mig.dist/1000),             datatype = "Gaussian", nboot = 1000, npermut = 0) ##population repeatability

lmm.wa <- lmer(winter.arrive ~ (1|col) +  (1 | id), data = filter(gull, !is.na(winter.arrive))%>% mutate(mig.dist = mig.dist/1000))
plot(lmm.wa)

##individual repeatability
#remove ids who change winter area
...
data <- filter(idvar.wa, !is.na(med.mig))
lm.wa <- lm(r~med.mig, data = data)
lm.wa.n <- lm(r~1, data = data)
lrt.wa <- anova(lm.wa.n, lm.wa) ## null

fall.mig.time <- filter(gull, !is.na(winter.arrive) & !is.na(col.depart))%>% mutate(mig.time.f = winter.arrive - col.depart) %>% summarise(mn = mean(mig.time.f), med = median(mig.time.f), max = max(mig.time.f), min = min(mig.time.f)) ## duration between col departure and winter arrive
...
data <- filter(idvar.wd, !is.na(med.mig))
lm.wd <- lm(r~med.mig, data =data)
lm.wd.n <- lm(r~1, data = data)
...
##does individual repeatability change with migration distance?
data <- filter(idvar.ca, !is.na(med.mig))
lm.ca <- lm(r~med.mig, data = data)
lm.ca.n <- lm(r~1, data = data)
lrt.ca <- anova(lm.ca.n, lm.ca)
anova(lm.ca.n, lm.ca)
...
plastic.id <-c(as.character(filter(sf.ol, sf_mn_overlap <= quantile(sf.ol$sf_mn_overlap, .05))$id),plastic.id)
...
Individuals used between `r min(gull$n.winter.p)` &ndash; `r max(gull$n.winter.p)` core areas during the non-breeding season. For all but five individuals (n = `r length(unique(gull$id)) - length(no.wa.overlap)`, `r round((length(unique(gull$id)) - length(no.wa.overlap))/length(unique(gull$id))*100,0)`%), winter areas overlapped across all years. These five individuals switched wintering areas between France and Western Sahara (n = 1), Mauritania and Portugal (n=1), France and UK (n = 1), and Morocco and UK (n = 2). Migration distance was therefore highly repeatable (R = `r myround(r.md$R$id,2)` [95% confidence interval: `r myround(r.md$CI_emp[1],2)` - `r myround(r.md$CI_emp[2],2)`]). `r ifelse(length(id.with.stop) == 62, "Sixty-two", "check code")` individuals (`r round(length(id.with.stop)/length(unique(gull$id))*100,0)`%) used a stopover in at least one year, and `r round(as.numeric(stopover.time.f)/sum(as.numeric(stopover.time.f),as.numeric(stopover.time.s))*100,0)`% of total time spent in stopover areas were most commonly used in autumn occurred before arrival to winter area. Use of stopover areas was less consistent than wintering areas: `r length(no.so.overlap$id)` out of `r length(id.with.stop)` individuals that used a stopover (`r round(length(no.so.overlap$id)/length(id.with.stop)*100)`%) had a stopover area that did not overlap among years (compared to `r round(length(no.wa.overlap)/length(unique(gull$id))*100)`% individuals who had non-overlapping wintering areas). 


Despite some variation in stopover area use, overlap in non-breeding distributions was generally high, with a median overlap of `r myround(median(wa.ol$mn_overlap),2)` (range: `r myround(min(wa.ol$mn_overlap),2)` &ndash; `r myround(max(wa.ol$mn_overlap),2)`). Non-breeding distributions were considerably more similar within individuals across years than between individuals (median between-individual overlap = `r myround(mdb.ol, 2)`, range = `r myround(min(rand.ol$overlap[rand.ol$pair == "Between"]), 2)`&ndash;`r myround(max(rand.ol$overlap[rand.ol$pair == "Between"]), 2)`,Fig. 2a and Fig. S3 in supplementary material), with none of the randomized sets producing a difference in median more extreme than the actual data (P `r ifelse(myround(pExtreme.ol, 3)<0.001, "< 0.001", "Check numbers")`). Site-fidelity within wintering areas was lower than non-breeding distribution overlap, with a median overlap of `r myround(median(gull$sf_mn_overlap),2)`, and differed substantially among individuals (range: `r min(gull$sf_mn_overlap)` – `r myround(max(gull$sf_mn_overlap),2)`, Fig. 2b). 


Individual variation in migration route was generally low in both autumn (range =  `r  round(min(wtraj.f$wvar),0)` &ndash; `r round(max(wtraj.f$wvar),0)` km, median = `r round(mdw.tf,0)` km) and spring (range =  `r round(min(wtraj.s$wvar),0)` &ndash; `r round(max(wtraj.s$wvar),0)` km, median = `r round(mdw.ts,0)` km). Most intra-individual route variation occurred at the Bay of Biscay and over the arid centre of Spain (Fig. 1). Intra-individual variation was significantly lower than variation found between paired individuals (autumn: between individual median = `r round(mdb.tf,0)` km, range = `r round(min(rand.t.f$mn_var[rand.t.f$pair == "Between"]),0)` &ndash; `r round(max(rand.t.f$mn_var[rand.t.f$pair == "Between"]),0)`; spring: between individual median = `r round(mdb.ts,0)` km, range = `r round(min(rand.t.s$mn_var[rand.t.f$pair == "Between"]),0)` &ndash; `r round(max(rand.t.s$mn_var[rand.t.f$pair == "Between"]),0)`; Fig. 2c-d and Fig. S3 in supplementary material) with probabilities less than 0.001 of obtaining a difference in medians as extreme from a randomized set. Examples of high and low non-breeding distribution overlap and route variation for a range of migration distances can be found in the supplementary material (Fig. S1 and S4).


Individuals departed their breeding colonies within a `r max(gull$col.depart, na.rm = T) - min(gull$col.depart, na.rm = T)`-day period between `r jultodate(min(gull$col.depart, na.rm = T))` &ndash; `r jultodate(max(gull$col.depart, na.rm = T))` (Fig. 3a). The median range of departure dates within an individual was `r median(range.cd$range)` days (range: `r min(range.cd$range)` &ndash;  `r max(range.cd$range)`) and was highly repeatable (`r myround(r.cd$R,2)` [`r myround(r.cd$CI_emp[1],2)` - `r myround(r.cd$CI_emp[2],2)`]). Arrival to wintering area occurred between `r jultodate(min(gull$winter.arrive, na.rm = T))` &ndash;  `r jultodate(max(gull$winter.arrive, na.rm = T))` (a period of `r max(gull$winter.arrive, na.rm = T) - min(gull$winter.arrive, na.rm = T)` days; Fig. 3b). Intra-individual arrival dates to wintering area ranged across years by `r min(range.wa$range)` &ndash;  `r max(range.wa$range)` days (median = `r median(range.wa$range)`) and repeatability was high (R = (`r myround(r.wa$R$id,2)` [`r myround(r.wa$CI_emp[1,1],2)`  - `r myround(r.wa$CI_emp[1,2],2)`]). During spring, the range of departure dates from wintering areas and arrival dates to colony within individuals was narrower than in autumn. Departure dates from wintering areas occurred within a `r max(gull$winter.depart) - min(gull$winter.depart)`-day period between `r jultodate(min(gull$winter.depart))` - `r jultodate(max(gull$winter.depart))` (Fig. 3c). The median intra-individual range of departure dates was `r median(range.wd$range)` days (range: `r min(range.wd$range)` &ndash;  `r max(range.wd$range)`). Repeatability of departure from wintering area was high (R = `r myround(r.wd$R$id,2)` [`r myround(r.wd$CI_emp[1,1],2)` - `r myround(r.wd$CI_emp[1,2],2)`]). Arrival to breeding colonies occurred between `r jultodate(min(gull$col.arrival))` &ndash;  `r jultodate(max(gull$col.arrival))` (a period of `r max(gull$col.arrival) - min(gull$col.arrival)` days; Fig. 3d). Intra-individual range in arrival dates to colony was `r min(range.ca$range)` &ndash;  `r max(range.ca$range)` days (median = `r median(range.ca$range)`). Repeatability was also high (R = `r myround(r.ca$R$id,2)` [`r myround(r.ca$CI_emp[1,1],2)` - `r myround(r.ca$CI_emp[1,2],2)`]). With the exception of departure date from colony, migration dates were later in longer distance migrants (Fig. 3, Table S2 in supplementary material). Colony explained little variance in timing of migration.  Linear mixed model results and partitioning of variance are reported in the supplementary material (Table S2). 


While most individuals tended to be highly consistent, for each behaviour a few individuals demonstrated high variation (Fig. 2, Fig. S5 in supplementary material). The individuals demonstrating the most variation were not the same for each behaviour, with `r n.variable` different individuals being in the upper 5th percentile of variation for at least one behaviour (Table S3 in supplementary material).


## Influence of migration distance on individual variation

Migration distances ranged from `r round(min(gull$mig.dist),0)` &ndash; `r round(max(gull$mig.dist),0)` km (median = `r round(median(gull$mig.dist),0)` km). Intra-individual non-breeding distribution overlap decreased with migration distance (overlap = `r myround(wa.ol.lm$coefficients[1],3)` + `r myround(wa.ol.lm$coefficients[2],6)`·migration distance, F(`r lrt.ol$Df[2]`, `r lrt.ol$Res.Df[2]`) = `r myround(lrt.ol$F [2],3)`, p = `r myround(lrt.ol$"Pr(>F)"[2],3)`; Fig. 2a). However, intra-individual variation increased at a significantly lower rate than the increase between individuals (residual overlap = `r myround(wa.dol.lm$coefficients[1],3)` + `r myround(wa.dol.lm$coefficients[2],3)`·migration distance, F(`r lrt.dol$Df[2]`, `r lrt.dol$Res.Df[2]`) = `r myround(lrt.dol$F[2],3)`, p = `r myround(lrt.dol$"Pr(>F)"[2],3)`; Fig s6a in supplemental material), suggesting that longer distance migrants were less variable in their behaviour than expected when considering the total space traversed during their movements (and vice-versa for shorter distance migrants). Intra-individual route variation increased slightly but significantly with migration distance in spring (variation = `r myround(lm.w.s$coefficients[1],3)` + `r myround(lm.w.s$coefficients[2],3)`·migration distance, F(`r lrt.w.s$Df[2]`, `r lrt.w.s$Res.Df[2]`) = `r myround(lrt.w.s$F[2],3)`, p = `r myround(lrt.w.s$"Pr(>F)"[2],3)`; Fig. 2d), even after accounting for increasing between-individual variation ( `r myround(dtraj.s.lm$coefficients[1],3)` + `r myround(dtraj.s.lm$coefficients[2],3)`·migration distance; F(`r lrt.dtraj.s$Df[2]`, `r lrt.dtraj.s$Res.Df[2]`) = `r myround(lrt.dtraj.s$F[2],3)`, p = `r myround(lrt.dtraj.s$"Pr(>F)"[2],3)`, Fig S6c in supplemental material). Autumn route variation did not change with migration distance  (F(`r lrt.w.f$Df[2]`, `r lrt.w.f$Res.Df[2]`) = `r myround(lrt.w.f$F[2],3)`, p = `r myround(lrt.w.f$"Pr(>F)"[2],3)`; Fig. 2c), nor did it significantly differ from variation observed between individuals (F(`r lrt.dtraj.f$Df[2]`, `r lrt.dtraj.f$Res.Df[2]`) = `r myround(lrt.dtraj.f$F[2],3)`, p = `r myround(lrt.dtraj.f$"Pr(>F)"[2],3)`, Fig S6b).


Winter site fidelity did not change significantly with migration distance (F(`r lrt.sf$Df[2]`, `r lrt.sf$Res.Df[2]`) = `r myround(lrt.sf$F[2],3)`, p = `r myround(lrt.sf$"Pr(>F)"[2],3)`, Fig2b). Migration distance also did not correlate with individual repeatability in departure date from colony (F(`r lrt.cd$Df[2]`, `r lrt.cd$Res.Df[2]`) = `r myround(lrt.cd$F[2],3)`, p = `r myround(lrt.cd$"Pr(>F)"[2],3)`; Fig. 2e), arrival to wintering area (F(`r lrt.wa$Df[2]`, `r lrt.wa$Res.Df[2]`) = `r myround(lrt.wa$F[2],3)`, p = `r myround(lrt.wa$"Pr(>F)"[2],3)`; Fig. 2f), departure from wintering area (F(`r lrt.wd$Df[2]`, `r lrt.wd$Res.Df[2]`) = `r myround(lrt.wd$F[2],3)`, p = `r myround(lrt.wd$"Pr(>F)"[2],3)`; Fig. 2g) or arrival to colony (F(`r lrt.ca$Df[2]`, `r lrt.ca$Res.Df[2]`) = `r myround(lrt.ca$F[2],3)`, p = `r myround(lrt.ca$"Pr(>F)"[2],3)`; Fig. 2h), suggesting individual consistency of migration timing did not increase with migration distance. 

```{r Table 1, include = T}
##n ids using migration distance
n.id.md <- table((gull %>% filter(!is.na(mig.dist)) %>% select(id, col) %>% distinct())$col)
##n seasons using migration distance
n.s.md <- table((gull %>% filter(!is.na(mig.dist)) %>% select(id, birdyear, col) %>% distinct())$col)
#n. individuals for route averaging
...
#n. seasons for route averaging
traj.seasons <- mn_traj %>% select(id, direction, n.years) %>% distinct() %>% left_join(gull %>% select(id, col) %>% distinct())

n.s.tf <- traj.seasons %>% filter(direction == "Autumn") %>% group_by(col) %>% summarise(n.seasons = sum(n.years)) %>% pull(n.seasons)
names(n.s.tf) <- traj.seasons %>% filter(direction == "Autumn") %>% group_by(col) %>% summarise(n.seasons = sum(n.years)) %>% pull(col)
n.s.ts <- traj.seasons %>% filter(direction == "Spring") %>% group_by(col) %>% summarise(n.seasons = sum(n.years)) %>% pull(n.seasons)
names(n.s.tf) <- traj.seasons %>% filter(direction == "Spring") %>% group_by(col) %>% summarise(n.seasons = sum(n.years)) %>% pull(col)
##Values per colony
...
p.direction <- ggplot(ol.df, aes(med.mig, resid.var)) + geom_blank() + 
  annotate("segment", x = 0.4, xend = 0.4, y = 0.29, yend = .13, size=.5, arrow=arrow(angle = 10, type = "closed", length = unit(0.1, "inches"))) + annotate("text", x = c(0,0, -.4), y = c(0.04,.35, .21),  label = c("     High", "Low", "Variation"), angle = 90, size = 3) + xlim(-1,1) + theme_void()


ol.df.pair <- rbind(rand.UD_overlap_range %>% select(overlap, med.mig, pair),
            ol.df %>% ungroup() %>% select(overlap = mn_overlap, med.mig) %>% mutate(pair = "within"))


p.ol <- ggplot(ol.df.pair, aes(med.mig, overlap, col = pair)) + geom_point(size = 1) + geom_smooth(method = "lm",se=F, size = 1) + scale_color_manual(values = c("grey75", "black"))  + xlab("Individual median migration distance (km)")  + ylab("Non-breeding distribution Overlap") + scale_x_continuous(expand = c(0, 0)) + ggtitle('a') +
...
p.wd <- ggplot(idvar.wd, aes(med.mig, r)) + geom_point(size = 1) +  ylab(expression(paste(R[i], "  of winter departures"))) + ylim(0,1) + scale_x_continuous(expand = c(0, 0)) + ggtitle('g') +
  xlab("Median migration distance (km)") + theme_bw() +   theme(axis.text =element_text(size=9),axis.title=element_text(size=9), plot.background = element_blank(), panel.grid=element_blank(), plot.title=element_text(hjust=0), plot.margin = margin(-3, 3, 0, 3)) 
...
p.r.cd <- ggplot(filter(gull.p, !is.na(col.depart)), aes(x = id, as.Date(col.depart-1, "1970-01-01"))) + 
 ...
 # geom_boxplot(aes(col = change.wa)) + 
  geom_boxplot(lwd = .75) + 
  geom_boxplot(data = filter(gull.p, !is.na(col.depart)) %>% mutate(col.depart = ifelse(change.wa ==T, col.depart, NA)), col = "white", fill = "grey",lwd = .75) + 
  ...
  scale_fill_gradientn(colours=c("#9d869e", "#979ebf","#93c6cf", "#ace089","#fff289"))+ 
...
p.r.wa <- ggplot(filter(gull.p, !is.na(winter.arrive)), aes(x =  id, as.Date(winter.arrive-1, "1970-01-01"))) + 
  ...
  geom_boxplot(lwd = .75) +  
  geom_boxplot(data = filter(gull.p, !is.na(winter.arrive)) %>% mutate(winter.arrive = ifelse(change.wa ==T, winter.arrive, NA)), col = "white", fill = "grey",lwd = .75) + 
...
p.r.wd <- ggplot(filter(gull.p, !is.na(winter.depart)), aes(x = id, as.Date(winter.depart-1, "1970-01-01"))) + 
  ...
  geom_boxplot(lwd = .75) +  
  geom_boxplot(data = filter(gull.p, !is.na(winter.depart)) %>% mutate(winter.depart = ifelse(change.wa ==T, winter.depart, NA)), col = "white", fill = "grey",lwd = .75) + 
  annotate("text", x = 23, y = as.Date(min(gull.p$winter.depart), "1970-01-01"), size = 4.5, 
...
p.r.ca <- ggplot(filter(gull.p, !is.na(col.arrival)), aes(x = id, as.Date(col.arrival-1, "1970-01-01"))) + 
 ...
 geom_boxplot(lwd = .75) +  
  geom_boxplot(data = filter(gull.p, !is.na(col.arrival)) %>% mutate(col.arrival = ifelse(change.wa ==T, col.arrival, NA)), col = "white", fill = "grey", lwd = .75) + 
  annotate("text", x = 65, y = as.Date(min(gull.p$col.arrival), "1970-01-01"), size = 4.5, label = paste0("R = ", myround(r.ca$R,2), "[", myround(r.ca$CI_emp[1],2), " - ", myround(r.ca$CI_emp[2],2), "]")) + 
  scale_y_date(breaks = as.Date(c(1,32,60,91,121,152,182,213,244,274,305,335,366, 397, 425, 456, 486, 517, 547)-1, "1970-01-01"), date_labels = "%b") +
...
n.id.tf <-  c(table((traj.seasons %>% filter(direction == "Autumn"))$n.years) , 0, 0)
n.id.ts <-  c(table((traj.seasons %>% filter(direction == "Spring"))$n.years) [1:4], 0, table((traj.seasons %>% filter(direction == "Spring"))$n.years)[5])
...
Figure S2 produced by LBBG_plasticity_pub.R
...
```{r Fig. S3, include = T}
#png("Figs3.png",  width = 16, height = 5.33, units = "cm", res = 600)
p.direction <- ggplot(ol.df, aes(med.mig, resid.var)) + geom_blank() + 
...
# pdf("Figure_S3.pdf",  width = 6.3, height = 2.1)
tiff("Fig_s3.tiff",  width = 20, height = 6, units = "cm", res = 300)
plot_grid(p.direction,bp.ol, bp.tf, bp.ts, nrow = 1, rel_widths = c(.1,1,1,1))
dev.off()
```
Figure S3.  A comparison of a) nonbreeding season overlap, b) autumn route variation and c) spring route variation within versus between individuals.
...
Table S2. Linear mixed model results for arrival and departure dates with individual as a random effect. T values are reported in brackets for the intercept. Individual variance is the variance of the individual-level random effect. Number of observations used in the models was 233, with 82 individuals. **All individuals (including switchers) were included, this was used to calculate repeatability (part 1)**
...
kable(summary(lmm.cd)$coef, digits = 5, caption = "Linear mixed model of individual colony departure date versus their median migration distance")
...
kable(summary(lmm.wa)$coef, digits = 5, caption = "Linear mixed model of individual winter arrival date versus their median migration distance")
...
kable(summary(lmm.wd)$coef, digits = 5, caption = "Linear mixed model of individual winter departure date versus their median migration distance")
...
kable(summary(lmm.ca)$coef, digits = 5, caption = "Linear mixed model of individual colony arrival date versus their median migration distance")
...
varid <- c(as.character((filter(ol.df, mn_overlap <= quantile(ol.df$mn_overlap, .05)) %>% select(id,mn_overlap) %>% arrange(mn_overlap))$id),
as.character((filter(sf.ol, sf_mn_overlap <= quantile(sf.ol$sf_mn_overlap, .05)) %>% select(id,sf_mn_overlap) %>% arrange(sf_mn_overlap))$id),
as.character((filter(wtraj.f, wvar >= quantile(wtraj.f$wvar, .95)) %>% select(id, wvar) %>% arrange(wvar))$id),
as.character((filter(wtraj.s, wvar >= quantile(wtraj.s$wvar, .95)) %>% select(id, wvar) %>% arrange(wvar))$id),
as.character((filter(idvar.cd, r <= quantile(idvar.cd$r, .05)) %>% select(id,r) %>% arrange(r))$id),
as.character((filter(idvar.wa, r <= quantile(idvar.wa$r, .05)) %>% select(id,r) %>% arrange(r))$id),
as.character((filter(idvar.wd, r <= quantile(idvar.wd$r, .05)) %>% select(id,r) %>% arrange(r))$id),
as.character((filter(idvar.ca, r <= quantile(idvar.ca$r, .05)) %>% select(id,r) %>% arrange(r))$id))
...
p.olr <- ggplot(ol.df, aes(med.mig, resid.var)) + geom_point(size = 1) + geom_smooth(method = "lm",se=F, size = 1, col = "black") + xlab("Median migration distance (km)")  + ylab("Residual Distribution Overlap") + scale_x_continuous(expand = c(0, 0)) + ggtitle('a') +
...
p.str <- ggplot(traj.s.w, aes(med.mig, resid.var)) + geom_point(size = 1) + 
  geom_smooth(method = "lm",se=F, size = 1, col = "black") + ggtitle('c') +
  ylab("Residual spring route variation (km)") + xlab("Median migration distance (km)")  + theme_bw() + scale_x_continuous(expand = c(0, 0)) +
...
p.ftr<- ggplot(traj.f.w,  aes(med.mig, resid.var)) + geom_point(size = 1)  + ggtitle('b') +
  ylab("Residual autumn route variation (km)") + xlab("Median migration distance (km)")  + theme_bw() + scale_x_continuous(expand = c(0, 0)) +
