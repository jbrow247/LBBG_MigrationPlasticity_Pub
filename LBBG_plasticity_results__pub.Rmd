...
gull <- gull %>% group_by(id) %>% mutate(mn.mig = mean(mig.dist), max.mig = max(mig.dist), n.yrs = length(mig.dist)) %>% arrange(mn.mig) %>% ungroup()
...
r.md <- rptR::rpt(mig.dist ~ (1 | id) , grname = c("id"), data = gull, 
            datatype = "Gaussian", nboot = 1000, npermut = 1000)

lmm.md <- lmer(mig.dist ~ (1 | id), data = gull)
...
idvar.md <- gull %>% select(id, mn.mig) %>% distinct() %>% full_join(idvar.md)
btwnvar <- VarCorr(lmm.md)[[1]][1]
idvar.md$r <- btwnvar/(btwnvar+idvar.md$idvar)
```

```{r wa site fidelity}
sf.ol<-gull %>% group_by(id) %>% select(id, mn.mig, sf_mn_overlap) %>% distinct()
sf.lm <- lm(sf_mn_overlap ~ mn.mig, data = sf.ol)
...
wa.ol<-gull %>% group_by(id) %>% mutate(max.n.wa = max(n.winter.p)) %>% select(id, mn.mig, mn_overlap, max_overlap, max.n.wa) %>% distinct()
wa.ol.lm <- lm(mn_overlap ~ mn.mig, data = wa.ol)
...
wtraj <- gull %>% group_by(id) %>% summarise(mn.mig = max(mig.dist, na.rm = T)) %>% right_join(wtraj)
wtraj <- wtraj %>% mutate(v.scale = wvar/mn.mig)
##spring
wtraj.s <- filter(wtraj, direction == "Spring")
lm.w.s<- lm(wvar ~ mn.mig, data = wtraj.s)
...
lm.w.f <- lm(wvar ~ mn.mig, data = wtraj.f)
anova(lm.w.f)
...
rand_traj_sum$mn.mig <- rowMeans(rand_traj_sum[,c("md1","md2")])
rand_traj_sum$pair <- "between"

###combine within and between data sets into a single df for plotting later on
traj.s <- rbind(wtraj.s %>% select( mn_var = wvar, mn.mig) %>% mutate(pair = "within"), 
                filter(rand_traj_sum, direction == "Spring") %>% select(mn_var, mn.mig, pair)) %>% arrange(pair)
traj.f <- rbind(wtraj.f %>% select( mn_var = wvar, mn.mig) %>% mutate(pair = "within"), 
                filter(rand_traj_sum, direction == "Autumn") %>% select(mn_var, mn.mig, pair)) %>% arrange(pair)

lm(mn_var ~ mn.mig, data = filter(traj.s, pair == 'between'))
lm(mn_var ~ mn.mig, data = filter(traj.s, pair == 'within')) 
lm(mn_var ~ mn.mig, data = filter(traj.f, pair == 'between')) 
lm(mn_var ~ mn.mig, data = filter(traj.f, pair == 'within')) #

##spring
##Calculate between relationship
traj.s.btwn.lm <- lm(mn_var ~ mn.mig, data = filter(traj.s, pair == 'between'))
traj.s.btwn.b <- coef(traj.s.btwn.lm)[1]
traj.s.btwn.m <- coef(traj.s.btwn.lm)[2]

traj.s.w <- filter(traj.s, pair == 'within')
traj.s.w$null <- traj.s.btwn.m*traj.s.w$mn.mig + traj.s.btwn.b
traj.s.w$resid.var <- traj.s.w$mn_var - traj.s.w$null

##check for significance
dtraj.s.lm <- lm(resid.var ~ mn.mig, data = traj.s.w)
...
#plot
ggplot(traj.s.w, aes(mn.mig, resid.var)) + geom_point()## variation still slightly more than would be expected in longer migrants
##Autumn
##Calculate between relationship
traj.f.btwn.lm <- lm(mn_var ~ mn.mig, data = filter(traj.f, pair == 'between'))
traj.f.btwn.b <- coef(traj.f.btwn.lm)[1]
traj.f.btwn.m <- coef(traj.f.btwn.lm)[2]

traj.f.w <- filter(traj.f, pair == 'within')
traj.f.w$null <- traj.f.btwn.m*traj.f.w$mn.mig + traj.f.btwn.b
traj.f.w$resid.var <- traj.f.w$mn_var - traj.f.w$null

##check for significance
dtraj.f.lm <- lm(resid.var ~ mn.mig, data = traj.f.w)
...
#plot
ggplot(traj.f.w, aes(mn.mig, resid.var)) + geom_point() ## variation still slightly more than would be expected in longer migrants
...
rand.UD_overlap_range$mn.mig <- rowMeans(rand.UD_overlap_range[,c("md1","md2")])
rand.UD_overlap_range$pair <- "between"

###combine within and between data sets into a single df for plotting later on
ol <- rbind(rand.UD_overlap_range %>% select(overlap, mn.mig, pair),
            wa.ol %>% ungroup() %>% select(overlap = mn_overlap, mn.mig) %>% mutate(pair = "within"))

##Calculate between relationship
ol.btwn.lm <- lm(overlap ~ mn.mig, data = filter(ol, pair == 'between'))
ol.btwn.b <- coef(ol.btwn.lm)[1]
ol.btwn.m <- coef(ol.btwn.lm)[2]

wa.ol$null <- ol.btwn.m*wa.ol$mn.mig + ol.btwn.b
wa.ol$resid.var <- wa.ol$mn_overlap - wa.ol$null

##check for significance
wa.dol.lm <- lm(resid.var ~ mn.mig, data = wa.ol)
...
##plot
ggplot(wa.ol, aes(mn.mig, resid.var)) + geom_point()
...
r.cd <- rptR::rpt(col.depart ~ (1 | id), grname = c("id"), data = filter(gull, !is.na(col.depart)), 
...
lmm.cd <- lmer(col.depart ~ (1 | id), data = filter(gull, !is.na(col.depart)))
plot(lmm.cd)
...
idvar.cd <- filter(gull, !is.na(col.depart))%>% select(id, mn.mig) %>% distinct() %>% full_join(idvar.cd)
...
lm.cd <- lm(r~mn.mig, data = idvar.cd)
...
r.wa <- rptR::rpt(winter.arrive ~ (1 | id) , grname = c("id"), data = filter(gull, !is.na(winter.arrive)) , 
...
lmm.wa <- lmer(winter.arrive ~ (1 | id), data = filter(gull, !is.na(winter.arrive)))
plot(lmm.wa)
...
idvar.wa <- filter(gull, !is.na(winter.arrive)) %>% select(id, mn.mig) %>% distinct() %>% full_join(idvar.wa)
...
lm.wa <- lm(r~mn.mig, data = idvar.wa)
...
idvar.wd <- gull %>% select(id, mn.mig) %>% distinct() %>% full_join(idvar.wd)
...
lm.wd <- lm(r~mn.mig, data = idvar.wd)
...
idvar.ca <- gull %>% select(id, mn.mig) %>% distinct() %>% full_join(idvar.ca)
btwnvar <- VarCorr(lmm.ca)[[1]][1]
idvar.ca$r <- btwnvar/(btwnvar+idvar.ca$idvar)

##does individual repeatability change with migration distance?
lm.ca <- lm(r~mn.mig, data = idvar.ca)
...
wa.ol.lm.b <- lm(overlap ~ mn.mig, data = filter(ol, pair == 'between'))

p.direction <- ggplot(wa.ol, aes(mn.mig, resid.var)) + geom_blank() + 
  annotate("segment", x = 0.4, xend = 0.4, y = 0.10, yend = .26, size=.5, arrow=arrow(angle = 10, type = "closed", length = unit(0.1, "inches"))) + annotate("text", x = c(0,0, -.4), y = c(0.04,.32, .18),  label = c("High", "Low", "Variation"), angle = 90, size = 3) + xlim(-1,1) + theme_void()

p.ol <- ggplot(ol, aes(mn.mig, overlap, col = pair)) + geom_point(size = 1) + geom_smooth(method = "lm",se=F, size = 1) + scale_color_manual(values = c("grey75", "black"))  + xlab("Individual mean migration distance (km)")  + ylab("Non-breeding distribution Overlap") + scale_x_continuous(expand = c(0, 0)) + ggtitle('a') +
...
p.sf <- ggplot(sf.ol, aes(mn.mig, sf_mn_overlap)) + geom_point(size = 1)   +  xlab("Individual mean migration distance (km)")  + ylab("Winter area overlap") + scale_x_continuous(expand = c(0, 0)) + ggtitle('b') +
  theme_bw() +  theme(axis.title.x = element_blank(), axis.text =element_text(size=9),axis.title.y=element_text(size=9), legend.position = "none", panel.grid=element_blank(), plot.title=element_text(hjust=0), plot.background = element_blank(), plot.margin = margin(0, 3, -3, 3)) ##with 95% confidence interval

p.st <- ggplot(traj.s, aes(mn.mig, mn_var, col = pair)) + geom_point(size = 1) + geom_smooth(method = "lm", se=F, size = 1) +
...
p.ft<-ggplot(traj.f, aes(mn.mig, mn_var, col = pair)) + geom_point(size = 1) + geom_smooth(data = filter(traj.f, pair == "between"), method = "lm", se=F, size = 1, col = "grey75") +
  scale_color_manual(values = c("grey75", "black")) + 
    xlab("Individual mean migration distance (km)") + ggtitle('c') +
...
p.cd <- ggplot(idvar.cd, aes(mn.mig, r)) + geom_point(size = 1) +  ylab(expression(paste(R[i], "  of colony departures"))) + ylim(0,1) + scale_x_continuous(expand = c(0, 0)) + ggtitle('e') +
  xlab("Individual mean migration distance (km)") + theme_bw() +    theme(axis.title.x=element_blank(),axis.text =element_text(size=9),axis.title.y=element_text(size=9), panel.grid=element_blank(), plot.title=element_text(hjust=0), plot.background = element_blank(), plot.margin = margin(-3, 3, -3, 3)) 

p.wa <- ggplot(idvar.wa, aes(mn.mig, r)) + geom_point(size = 1) +  ylab(expression(paste(R[i], "  of winter arrivals"))) + ylim(0,1) + scale_x_continuous(expand = c(0, 0)) + ggtitle('f') +
  xlab("Individual mean migration distance (km)") + theme_bw() +    theme(axis.title.x = element_blank(),axis.text =element_text(size=9),axis.title.y=element_text(size=9),plot.background = element_blank(), panel.grid=element_blank(), plot.title=element_text(hjust=0), plot.margin = margin(-3, 3, -3, 3))

p.wd <- ggplot(idvar.wd, aes(mn.mig, r)) + geom_point(size = 1) +  ylab(expression(paste(R[i], "  of winter departures"))) + ylim(0,1) + scale_x_continuous(expand = c(0, 0)) + ggtitle('g') +
...
p.ca <- ggplot(idvar.ca, aes(mn.mig, r)) + geom_point(size = 1) +  ylab(expression(paste(R[i], "  of colony arrivals"))) + ylim(0,1) + ggtitle('h') + scale_x_continuous(expand = c(0, 0)) +
  xlab("Mean migration distance (km)") + theme_bw() +   theme(axis.text =element_text(size=9),axis.title=element_text(size=9), plot.background = element_blank(), panel.grid=element_blank(), plot.title=element_text(hjust=0), plot.margin = margin(-3, 3, 0, 3)) 
...
```{r Fig. 3, include=T}
p.r.cd <- ggplot(filter(gull, !is.na(col.depart)), aes(x = reorder(id, mig.dist, max), as.Date(col.depart-1, "1970-01-01"))) + 
  geom_boxplot() + annotate("text", x = 25, y = as.Date(min(gull$col.depart, na.rm = T), "1970-01-01"), size = 3,label = paste0("R = ", myround(r.cd$R,2), "[", myround(r.cd$CI_emp[1],2), " - ", myround(r.cd$CI_emp[2],2), "]")) + 
...
p.r.wa <- ggplot(filter(gull, !is.na(winter.arrive)), aes(x = reorder(id, mig.dist, max), as.Date(winter.arrive-1, "1970-01-01"))) + 
  geom_boxplot() +  
  annotate("text", x = 65, y = as.Date(min(gull$winter.arrive, na.rm=T), "1970-01-01"), size = 3,
...
p.r.ca <- ggplot(gull, aes(x = reorder(id, mig.dist, max), as.Date(col.arrival-1, "1970-01-01"))) + 
  geom_boxplot() +  annotate("text", x = 65, y = as.Date(min(gull$col.arrival), "1970-01-01"), size = 3, label = paste0("R = ", myround(r.ca$R,2), "[", myround(r.ca$CI_emp[1],2), " - ", myround(r.ca$CI_emp[2],2), "]")) + 
  scale_y_date(breaks = as.Date(c(1,32,60,91,121,152,182,213,244,274,305,335,366, 397, 425, 456, 486, 517, 547)-1, "1970-01-01"), date_labels = "%b") +
...
p.direction <- ggplot(wa.ol, aes(mn.mig, resid.var)) + geom_blank() + 
  annotate("segment", x = 0, xend = 0, y = 0.13, yend = .24, size=.5, arrow=arrow(angle = 10, type = "closed")) + annotate("text", x = c(0,0), y = c(0.02,.35),  label = c("High variation", "Low variation"), angle = 90, size = 3) + xlim(-1,1) + theme_void()

p.olr <- ggplot(wa.ol, aes(mn.mig, resid.var)) + geom_point(size = 1) + geom_smooth(method = "lm",se=F, size = 1, col = "black") + xlab("Mean migration distance (km)")  + ylab("Residual Distribution Overlap") + scale_x_continuous(expand = c(0, 0)) + ggtitle('a') +
...
p.str <- ggplot(traj.s.w, aes(mn.mig, resid.var)) + geom_point(size = 1) + ggtitle('c') +
...
p.ftr<-ggplot(traj.f.w,  aes(mn.mig, resid.var)) + geom_point(size = 1) + ggtitle('b') +
  ylab("Residual spring route variation (km)") + xlab("Mean migration distance (km)")  + theme_bw() + scale_x_continuous(expand = c(0, 0)) +
